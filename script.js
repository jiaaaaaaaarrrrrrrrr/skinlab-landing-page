...
class DataHandler {
  ...
  // 新增方法：从成功页面提交数据
  async submitSuccessPageData(data) {
    if (this.isSubmitting) {
      throw new Error('正在提交中，请稍候...');
    }
    ...
    try {
      console.log('从成功页面提交数据到Google Sheets:', data);
      
      // 格式化数据以匹配Google Sheets
      const formattedData = {
        "时间戳": data.timestamp || this.getCurrentDateTime(),
        "预约ID": data.bookingId || `SKIN-OFFLINE-${Date.now()}`,
        "姓名": data.name || '',
        "手机号码": data.phone || '',
        "邮箱": data.email || '',
        "皮肤问题": data.concern || '',
        "体验周次": data.week || '',
        "提交时间": data.submitTime || this.getCurrentDateTime(),
        "客户电话": data.phone || '',
        "名额": "已预约",
